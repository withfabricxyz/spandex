...
SpanDEX provides React hooks for integrating swaps into your React applications. The hooks are
...
```bash [npm]
npm i @withfabric/spandex-react wagmi viem
```

```bash [bun]
bun i @withfabric/spandex-react wagmi viem
```

```bash [pnpm]
pnpm i @withfabric/spandex-react wagmi viem
```
...
Wrap your application with `SpandexProvider`, within the WagmiProvider:
...
function App() {
  return (
    <WagmiProvider config={wagmiConfig}>
      <SpandexProvider config={spandexConfig}>
        <YourApp />
      </SpandexProvider>
    </WagmiProvider>
  );
...
The `SpandexProvider` accepts a `config` prop with two properties:

- **`aggregators`**: Array of aggregator configurations (see [Getting Started](/configuration.mdx))
- **`options`**: Optional default options including strategy (see [Strategies](/strategies.mdx))
...
type UseQuotesParams = {
  ...
  inputAmount?: bigint; // Required for exactIn
  outputAmount?: bigint; // Required for targetOut
  slippageBps: number;
  // Optional overrides:
