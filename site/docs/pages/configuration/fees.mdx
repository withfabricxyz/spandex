...
There are several different fee layers to consider in spanDEX, even though spanDEX itself imposed no fees:

- **Provider fees**: costs imposed the upstream provider and included in the quote.
...
- **LP fees**: Fees paid to market makers (AMM, RFQ, etc) and are included in the quote.
- **Network fees**: Gas costs, which scale with the complexity of the route. Not included in the quote, but estimated during simulation.

## Provider Fees

These fees are represent the default settings for any given provider (new account, or anonymous) and are subject to negitation.
...
Additionally each swap has network fees, which are not included in `outputTokens`. However, the gas usage can be factored in when selecting quotes.

## Integrator Fee Capture (Optional)

spanDEX supports two integrator capture for applications which source order flow from end users:
...
<Callout type="info">
  If you have negotiated features like surplus for a given provider, you can activate that feature for any provider using the `negotiatedFeatures` option.
</Callout>
