...
1. `AggregatorProxy` serializes swap params into a query string and performs a GET to your endpoint.
1. Service fetches provider quotes and streams them immediately as each one resolves via
   `newQuoteStream(...)`.
1. Client decodes the stream with `decodeQuoteStream(...)` into quote promises.
1. Those promises resolve incrementally, so UI/state can update as results arrive.
