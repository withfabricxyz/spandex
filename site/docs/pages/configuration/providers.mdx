...
<div className="provider-grid">
  <a className="provider-card" href="/providers/fabric">
    ...
    <span className="provider-card__name">Fabric (deprecated)</span>
  </a>
  ...
  <a className="provider-card" href="/providers/fynd">
    <span className="provider-card__logo"><FyndLogo /></span>
    <span className="provider-card__name">Fynd</span>
  </a>
  ...
  <a className="provider-card" href="/providers/mobula">
    <span className="provider-card__logo"><MobulaLogo /></span>
    <span className="provider-card__name">Mobula</span>
  </a>
...
<div className="provider-grid">
  <a className="provider-card" href="/providers/odos">
    ...
    <span className="provider-card__name">Odos (deprecated)</span>
  </a>
</div>

<Callout type="warning">
  Odos is shutting down. Its compatibility provider warns on construction and returns a failed
  quote immediately. Remove `odos()` from your configuration.
</Callout>
...
- Provider-imposed fees (surplus taking or swap fees)
...
```ts
import { createConfig, nordstern, relay, velora } from "@spandex/core";

export function configForRegion(region: string) {
  if (region.startsWith("eu")) {
    return createConfig({
      providers: [nordstern({}), relay({})],
    });
  }

  return createConfig({
    providers: [nordstern({}), velora({})],
  });
}
```
