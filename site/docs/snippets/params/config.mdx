...
See [Proxy Mode guide](/configuration/proxy) for service architecture and streaming details, including
`delegatedActions` (`getQuotes`/`getQuote`) to control which top-level APIs are delegated to proxy.
For the managed deployment helper, see [Cloud Mode (beta)](/configuration/cloud).
