...
|----------|-----------|----------|
| `zeroX` | yes | [View](/providers/0x) |
| `fabric` (deprecated; always rejects) | no | [Migration details](/providers/fabric) |
| `fynd` | hosted: yes, self-hosted: no | [View](/providers/fynd) |
| `kyberswap` | no | [View](/providers/kyberswap) |
| `odos` (deprecated; always fails) | no | [Migration details](/providers/odos) |
| `mobula` | yes | [View](/providers/mobula) |
...
See [Proxy Mode guide](/configuration/proxy) for service architecture and streaming details. In
proxy mode, you must provide at least one `delegatedActions` entry. Those names match both the
delegated function names and the route suffixes on the service. `getQuotes` and `getQuote` compose
locally on the client. For the managed deployment helper, see [Cloud Mode (beta)](/configuration/cloud).
...
When using proxy mode, server-streamed simulated quotes do not require a local client for
`getQuotes`/`getQuote`. Local clients are still useful if you call lower-level simulation utilities
directly.

Note: The public client is used to simulate quotes and validate them onchain. This requires an RPC provider which supports this, and many public providers do not. Please ensure your viem client is setup with a compatible RPC provider (e.g. Alchemy, Infura, etc). Additionally, you can setup RPC failover in your client configuration to improve reliability of quote fetching and simulation. See [viem documentation](https://viem.sh/docs/clients/transports/fallback) for more details.
