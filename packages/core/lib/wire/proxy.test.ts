...
describe("proxy delegatedActions config", () => {
  it("does not delegate getQuote/getQuotes by default", () => {
    const instance = proxy({ pathOrUrl: "https://example.com/api" });

    expect(instance.isDelegatedAction("getQuote")).toBe(false);
    expect(instance.isDelegatedAction("getQuotes")).toBe(false);
  });

  it("supports delegatedActions and deferredActions (deprecated) with delegatedActions precedence", () => {
    const delegated = proxy({
      pathOrUrl: "https://example.com/api",
      delegatedActions: ["getQuotes"],
    });
    expect(delegated.isDelegatedAction("getQuotes")).toBe(true);
    expect(delegated.isDelegatedAction("getQuote")).toBe(false);

    const deprecated = proxy({
      pathOrUrl: "https://example.com/api",
      deferredActions: ["getQuote"],
    });
    expect(deprecated.isDelegatedAction("getQuote")).toBe(true);
    expect(deprecated.isDelegatedAction("getQuotes")).toBe(false);
    ...
    expect(both.isDelegatedAction("getQuotes")).toBe(true);
    expect(both.isDelegatedAction("getQuote")).toBe(false);
  });
});
