...
export type AggregatorProxyConfig = {
  ...
  /**
   * Optional list of actions to delegate to the proxy instead of executing locally.
   * By default only prepareQuotes is delegated when a proxy is active.
   */
  delegatedActions?: ("getQuotes" | "getQuote")[];

  /**
   * @deprecated Use delegatedActions instead.
   */
...
export class AggregatorProxy {
  private readonly delegatedActions: Set<"getQuotes" | "getQuote">;
  constructor(private config: AggregatorProxyConfig) {
    const delegatedActions = config.delegatedActions ?? config.deferredActions ?? [];
    this.delegatedActions = new Set(delegatedActions);
  }

  /**
   * Determines if a given action should be delegated to the proxy based on the configuration.
   *
   * By default prepareQuotes is treated as a delegated call, while getQuotes and getQuote are not.
   ...
   * @returns True if the method should be delegated to the proxy, false otherwise.
   */
  isDelegatedAction(method: "getQuotes" | "getQuote"): boolean {
    return this.delegatedActions.has(method);
  }

  /**
   * @deprecated Use isDelegatedAction(method) instead.
   */
  isDeferredAction(method: "getQuotes" | "getQuote"): boolean {
    return this.isDelegatedAction(method);
  }
